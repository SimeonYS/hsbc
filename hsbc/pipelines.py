...
class HhsbcPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hsbc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hsbc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hsbc (date, title, link, content)"
